...
def train_and_get_error(test_data,  num_species, saved_params, values_to_predict,
    ...
    try:
        ...
        raise RuntimeError("Couldn't find model %s in the model_factory" % model_name) from e
    else:
...
def get_data(path_to_xyz_file, path_to_saved_weights, path_to_y_vals, logger, num_dist_basis=40, 
    ...
    with gzip.open(path_to_saved_weights, 'rb') as f:
        try:
            ...
            f.seek(0)
            saved_params = pickle.load(f, encoding='latin1')
    ...
    Y_vals = np.load(path_to_y_vals, allow_pickle=True)
    Emean = Y_vals['Y_mean']
    Estd = Y_vals['Y_std']
...
def main(**params):
    ...
    mydir = os.path.join(parent_dir, timestamp)
