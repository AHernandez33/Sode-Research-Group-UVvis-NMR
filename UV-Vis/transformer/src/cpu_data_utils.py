...
def load_data_from_df(dataset_path, add_dummy_node=True, one_hot_formal_charge=False, use_data_saving=True,two_d_only=False):
    ...
    """
    feat_stamp = (
        f'{"_dn" if add_dummy_node else ""}'
        f'{"_ohfc" if one_hot_formal_charge else ""}'
        f'{"_2d" if two_d_only else "_3d"}'
        "_uvvis181"
    )
    ...
    wavelength_columns = [
        str(wavelength)
        for wavelength in range(220, 401)
    ]
    ...
    if missing_columns:
        raise ValueError(
            "The Transformer dataset is missing wavelength columns: "
            + ", ".join(missing_columns)
        )

    if "smiles" in data_df.columns:
        data_x = data_df["smiles"].astype(str).values
    else:
        data_x = data_df.iloc[:, 0].astype(str).values
    ...
    if data_y.shape[1] != 181:
        raise ValueError(
            "Expected 181 UV-Vis target values from 220 through 400 nm, "
            f"but found {data_y.shape[1]}."
        )

    print(
        "Loaded Transformer targets:",
        data_y.shape
    )
...
def mol_collate_func(batch):
    ...
    for molecule in batch:
        ...
        if label.shape[0] != 181:
            raise ValueError(
                "Each Transformer target must contain exactly 181 values. "
                f"Found {label.shape[0]}."
            )
    ...
    if batch_tensors[3].ndim != 2 or batch_tensors[3].shape[1] != 181:
        raise ValueError(
            "Batched Transformer targets must have shape "
            "(batch_size, 181). "
            f"Found {tuple(batch_tensors[3].shape)}."
        )
